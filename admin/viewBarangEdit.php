<?php 
	...
	if (isset($_POST['getSimpan'])) {
		...
		if ($kode_barang == " " || $nama_barang == " " || $merek_barang == " " || $distributor == " "|| $tanggal_masuk == " " || $harga == " " || $stok == " " || $foto == " " || $ket == " " ) {
			...
			if ($harga < 0 || $stok < 0) {
				...
				if ($_FILES['foto']['name'] == "") {
					$value = "kd_barang='$kode_barang',nama_barang='$nama_barang',kd_merek='$merek_barang',kd_distributor='$distributor',tanggal_masuk='$tanggal_masuk',harga_barang='$harga',stok_barang='$stok',keterangan='$ket'";
					$response = $br->update($table,$value,"kd_barang",$_GET['id'],"?page=viewBarang");
				}else{
					$response = $br->validateImage();
					if ($response['types'] == "true") {
						$value = "kd_barang='$kode_barang',nama_barang='$nama_barang',kd_merek='$merek_barang',kd_distributor='$distributor',tanggal_masuk='$tanggal_masuk',harga_barang='$harga',stok_barang='$stok',keterangan='$ket',gambar='$response[image]'";
						$response = $br->update($table,$value,"kd_barang",$_GET['id'],"?page=viewBarang");
					}else{
            			...
            			<div class="card">
                        	...
                        	<div class="card-body">
                        		<div class="col-12">
                                    ...
                                    <input type="text" class="form-control" name="tanggal_masuk" value="<?php echo @$data['tanggal_masuk'] ?>">
                                </div>
											<div class="form-group">
